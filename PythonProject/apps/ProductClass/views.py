...
class FrontAddView(BaseView):  # 前台商品类别添加
    ...
    def post(self, request):
        ...
        productClass.productClassName = request.POST.get('productClass.productClassName')
        productClass.save() # 保存商品类别信息到数据库
        return JsonResponse({'success': True, 'message': '保存成功'})


class FrontModifyView(BaseView):  # 前台修改商品类别
    def get(self, request, productClassId):
        context = {'productClassId': productClassId}
...
class FrontShowView(View):  # 前台显示商品类别详情页
    def get(self, request, productClassId):
        # 查询需要显示的商品类别对象
        productClass = ProductClass.objects.get(productClassId=productClassId)
        context = {
...
class UpdateView(BaseView):  # Ajax方式商品类别更新
    ...
    def post(self, request, productClassId):
...
class DeletesView(BaseView):  # 删除商品类别信息
    ...
    def handle(self, request):
        productClassIds = self.getStrParam(request, 'productClassIds')
        productClassIds = productClassIds.split(',')
        count = 0
        try:
            for productClassId in productClassIds:
                ProductClass.objects.get(productClassId=productClassId).delete()
                count = count + 1
...
class OutToExcelView(BaseView):  # 导出商品类别信息到excel并下载
    def get(self, request):
        ...
        # 设置要导入到excel的列
        columns_map = {
            'productClassId': '商品类别编号',
            'productClassName': '商品类别名称',
        }
